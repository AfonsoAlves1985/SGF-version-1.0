...
export const contractAlertTypeEnum = pgEnum("contract_alert_type", [
  "monthly_payment",
  "contract_expiry",
]);
...
export const roomReservationStatusEnum = pgEnum("room_reservation_status", [
  "confirmada",
  "pendente",
  "cancelada",
]);
...
export const consumableMonthlyMovements = pgTable(
  "consumable_monthly_movements",
  {
...
export const consumableWeeklyMovements = pgTable(
  "consumable_weekly_movements",
  {
    id: serial().primaryKey(),
    consumableId: integer().notNull(),
    spaceId: integer().notNull(),
...
export const contractsWithSpace = pgTable(
  "contracts_with_space",
  {
    id: serial().primaryKey(),
  ...
  table => [index("fk_1").on(table.spaceId)]
);
...
export const maintenanceRequests = pgTable("maintenance_requests", {
  ...
  description: text(),
  requesterName: varchar({ length: 255 }),
  ...
  spaceId: integer()
    .default(1)
    .notNull()
    .references(() => maintenanceSpaces.id, { onDelete: "cascade" }),
});
...
export const suppliersWithSpace = pgTable(
  "suppliers_with_space",
  {
    id: serial().primaryKey(),
    spaceId: integer().references(() => supplierSpaces.id, {
      onDelete: "cascade",
    }),
    companyName: varchar({ length: 255 }).notNull(),
    serviceTypes: json().notNull(),
  ...
  table => [
    index("suppliers_with_space_spaceId_consumable_spaces_id_fk").on(
      table.spaceId
    ),
  ]
);
...
export const purchaseRequests = pgTable(
  "purchase_requests",
  {
    ...
    externalRequestId: varchar({ length: 120 }),
    externalApprovalStatus: varchar({ length: 40 }),
    externalApprovedBy: varchar({ length: 255 }),
    externalApprovedAt: timestamp(),
    externalApprovalReason: text(),
    externalApprovalPayload: json(),
    integrationWebhookAttempts: integer().default(0).notNull(),
    integrationWebhookLastAttemptAt: timestamp(),
    integrationWebhookLastDeliveredAt: timestamp(),
    integrationWebhookLastStatus: varchar({ length: 40 }),
    integrationWebhookLastStatusCode: integer(),
    integrationWebhookLastError: text(),
    integrationCallbackAttempts: integer().default(0).notNull(),
    integrationCallbackLastAt: timestamp(),
    integrationCallbackLastStatus: varchar({ length: 40 }),
    integrationCallbackLastDecision: varchar({ length: 20 }),
...
export type InsertConsumableWithSpace =
  typeof consumablesWithSpace.$inferInsert;
export type ConsumableWithSpace = typeof consumablesWithSpace.$inferSelect;

export type InsertConsumableWeeklyMovement =
  typeof consumableWeeklyMovements.$inferInsert;
export type ConsumableWeeklyMovement =
  typeof consumableWeeklyMovements.$inferSelect;

export type InsertConsumableMonthlyMovement =
  typeof consumableMonthlyMovements.$inferInsert;
export type ConsumableMonthlyMovement =
  typeof consumableMonthlyMovements.$inferSelect;

export type InsertConsumableStockAuditLog =
  typeof consumableStockAuditLog.$inferInsert;
export type ConsumableStockAuditLog =
  typeof consumableStockAuditLog.$inferSelect;
