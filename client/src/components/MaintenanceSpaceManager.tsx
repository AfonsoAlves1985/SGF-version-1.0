...
export interface MaintenanceSpace {
  id: number;
  name: string;
  description?: string;
}

export interface MaintenanceSpaceManagerProps {
  spaces: MaintenanceSpace[];
  selectedSpace: number | null;
  onSelectSpace: (spaceId: number) => void;
  onCreateSpace: (data: { name: string; description?: string }) => void;
  onUpdateSpace: (id: number, data: { name?: string; description?: string }) => void;
...
export function MaintenanceSpaceManager({
  spaces,
  selectedSpace,
  onSelectSpace,
  onCreateSpace,
  ...
  const handleOpenDialog = (space?: MaintenanceSpace) => {
    if (space) {
  ...
  const handleDelete = (spaceId: number) => {
    if (window.confirm("Tem certeza que deseja deletar esta unidade? Todas as requisições de manutenção associadas serão removidas.")) {
